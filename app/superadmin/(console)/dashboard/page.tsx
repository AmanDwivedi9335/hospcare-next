      ...
      <main className="flex-1 space-y-6 bg-slate-50 p-6">
        <section className="grid gap-4 lg:grid-cols-4">
          {incomeWidgets.map((widget) => (
            <div key={widget.title} className="rounded-2xl border border-white/60 bg-white p-4 shadow-sm shadow-slate-200">
          ...
          <div className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6">
            ...
            <div className="grid gap-3 text-sm text-slate-600">
              {quickMetrics.map((metric) => (
                <div key={metric.title} className="flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
                  <div>
          ...
          <div className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6">
            ...
            <div className="grid grid-cols-7 gap-4 text-center text-sm">
              {calendarDays.map((day) => (
                <div
                  key={day.day}
                  className={`rounded-2xl border p-4 ${day.active ? "border-emerald-200 bg-emerald-50" : "border-slate-100 bg-slate-50"}`}
                >
              ...
              {patientTypes.map((type) => (
                <div key={type.label} className="space-y-1">
                  ...
                  <div className="h-2 rounded-full bg-slate-100">
                    <div className="h-full rounded-full bg-emerald-500" style={{ width: `${Math.min(100, (type.value / 400) * 100)}%` }} />
                  </div>
                </div>
              ))}
            </div>
